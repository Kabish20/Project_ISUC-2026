...
import {
  Building2, Star, Users, CheckCircle2, Zap, ArrowRight,
  Hotel, Plane, Utensils, Car, Award
} from 'lucide-react';
import { SITE_CONFIG } from '@/constants/siteConfig';
import { EXHIBIT_TABLE_FEATURES, TOUR_INCLUSIONS } from '@/data';
import { CountUp } from '@/components/common/CountUp';
import { FeatureCard } from '@/components/common/FeatureCard';
import { ExhibitModal } from '@/components/modals/ExhibitModal';

const iconMap = {
  Building2: <Building2 className="w-6 h-6 text-primary-400" />,
  Users: <Users className="w-6 h-6 text-accent-400" />,
...
/**
 * Complete Exhibitor Landing Section & Packages Breakdown
 */
export const ExhibitSection = ({ isModalOpen, setIsModalOpen }) => {
  ...
  return (
    <div id="exhibit" ref={containerRef} className="relative bg-secondary-900 text-white selection:bg-primary-500/30">
      ...
      <div className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {[...Array(3)].map((_, i) => (
            <motion.div
              key={i}
              className="absolute w-72 h-72 rounded-full bg-primary-500/5 blur-3xl"
              style={{
                top: `${20 + i * 30}%`,
                left: `${10 + i * 35}%`,
              }}
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative z-10 text-center px-4 max-w-6xl mx-auto">
          ...
          >
            ...
            Secure one of <span className="text-primary-400 font-bold">{SITE_CONFIG.stats.totalSuppliersCap} exclusive tables</span> and meet India's top 300 buyers in focused, pre-scheduled sessions.
          </motion.p>
          ...
          >
            ...
            <button
              ...
              className="inline-flex items-center gap-3 px-8 py-4 text-base font-semibold text-white border border-white/20 rounded-xl hover:bg-white/10 backdrop-blur-sm transition-all duration-300 cursor-pointer"
            >
              Explore Inclusions
            </button>
          </motion.div>
      ...
      <div id="details" className="py-32 relative">
        <div className="container mx-auto px-4">
            ...
            >
              ...
              <div className="absolute -bottom-6 -right-6 z-20 bg-secondary-900 border border-white/10 p-6 rounded-2xl shadow-2xl hidden md:block">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-2xl font-bold text-white tracking-tight">{SITE_CONFIG.stats.meetingsPerSupplier}</div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-32">
            {EXHIBIT_TABLE_FEATURES.map((feature, i) => (
              <FeatureCard
                key={i}
                icon={iconMap[feature.iconName] || <Building2 className="w-6 h-6 text-primary-400" />}
                title={feature.title}
                desc={feature.desc}
                delay={i * 0.1}
              />
              ...
              >
                <div className="w-16 h-16 rounded-2xl bg-primary-900/30 flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300">
                  <div className="text-primary-400">
                    {iconMap[item.iconName] || <Hotel className="w-5 h-5" />}
                  </div>
                </div>
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          >
            <h2 className="text-3xl md:text-5xl font-display font-bold text-white mb-8">
              Limited to {SITE_CONFIG.stats.totalSuppliersCap} Tables. <br />
              <span className="text-primary-400">Claim Your City Rotation.</span>
            </h2>
            <p className="text-secondary-300 text-lg mb-10 max-w-2xl mx-auto">
              Meet pre-screened Indian Umrah buyers in Mumbai, Chennai, and Delhi with full tour management included.
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
              className="px-10 py-5 rounded-2xl bg-primary-600 hover:bg-primary-500 text-white font-bold text-lg shadow-xl shadow-primary-600/30 transition-all hover:-translate-y-1 cursor-pointer"
            >
              Open Booking Application Form →
            </button>
          </motion.div>
        </div>
      </div>
      ...
      {/* Lightbox Modal */}
      <ExhibitModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  );
};
